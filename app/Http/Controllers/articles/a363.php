<link rel='stylesheet' href='digital_ocean.css'><br><img src=https://community-cdn-digitalocean-com.global.ssl.fastly.net/assets/tutorials/images/large/bacula_twitter.png?1433958956/> <br> 
...
<p>In this tutorial, we will show you how to install and configure the server components of Bacula on a CentOS 7 server. We will configure Bacula to perform a weekly job that creates a local backup (i.e. a backup of its own host). This, by itself, is not a particularly compelling use of Bacula, but it will provide you with a good starting point for creating backups of your other servers, i.e. the backup clients. The next tutorial in this series will cover creating backups of your other, remote, servers by installing and configuring the Bacula client, and configuring the Bacula server.</p>

<p>If you'd rather use Ubuntu 14.04 instead, follow this link: <a href="https://indiareads/community/tutorials/how-to-install-bacula-server-on-ubuntu-14-04">How To Install Bacula Server on Ubuntu 14.04</a>.</p>

<h2 id="prerequisites">Prerequisites</h2>

<p>You must have superuser (sudo) access on a CentOS 7 server. Also, the server will require adequate disk space for all of the backups that you plan on retaining at any given time.</p>
...
<p>The last assumption is that SELinux is disabled or you are able to troubleshoot SELinux-related issues on your own.</p>
...
<h2 id="install-bacula-and-mysql">Install Bacula and MySQL</h2>

<p>Bacula uses an SQL database, such as MySQL or PostreSQL, to manage its backups catalog. We will use MariaDB, a drop-in replacement for MySQL, in this tutorial.</p>

<p>Install the Bacula and MariaDB Server packages with yum:</p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo yum install -y bacula-director bacula-storage bacula-console bacula-client mariadb-server
</li></ul></code></pre>
<p>When the installation is complete, we need to start MySQL with the following command:</p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo systemctl start mariadb
</li></ul></code></pre>
<p>Now that MySQL (MariaDB) is installed and running, let's create the Bacula database user and tables, with these scripts:</p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">/usr/libexec/bacula/grant_mysql_privileges
</li><li class="line" prefix="$">/usr/libexec/bacula/create_mysql_database -u root
</li><li class="line" prefix="$">/usr/libexec/bacula/make_mysql_tables -u bacula
</li></ul></code></pre>
<p>Next, we want to run a simple security script that will remove some dangerous defaults and lock down access to our database system a little bit. Start the interactive script by running:</p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo mysql_secure_installation
</li></ul></code></pre>
<p>The prompt will ask you for your current root password. Since you just installed MySQL, you most likely won’t have one, so leave it blank by pressing enter. Then the prompt will ask you if you want to set a root password. Go ahead and hit <code>Enter</code>, and set the password. For the rest of the questions, you should simply hit the <code>Enter</code> key through each prompt to accept the default values. This will remove some sample users and databases, disable remote root logins, and load these new rules so that MySQL immediately respects the changes we have made.</p>
...
<p>Enter the MySQL root password, that you just set, at the prompt.</p>

<p>Now set the password for the Bacula database user. Use this command, but replace the highlighted "bacula<em>db</em>password" with strong password:</p>
<pre class="code-pre custom_prefix"><code langs=""><ul class="prefixed"><li class="line" prefix="MariaDB [(none)]>">UPDATE mysql.user SET Password=PASSWORD('<span class="highlight">bacula_db_password</span>') WHERE User='bacula';
</li><li class="line" prefix="MariaDB [(none)]>">FLUSH PRIVILEGES;
</li></ul></code></pre>
...
<h3 id="set-bacula-to-use-mysql-library">Set Bacula to Use MySQL Library</h3>
...
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo alternatives --config libbaccats.so
</li></ul></code></pre>
<p>You will see the following prompt. Enter 1 (MySQL):</p>
<pre class="code-pre "><code langs=""><div class="secondary-code-label " title="Output">Output</div>There are 3 programs which provide 'libbaccats.so'.
...
Enter to keep the current selection[+], or type selection number: <span class="highlight">1</span>
</code></pre>
...
<h3 id="configure-director-resource">Configure Director Resource</h3>

<p>Find the Director resource, and configure it to listen on <code>127.0.0.1</code> (localhost), by adding the <code>DirAddress</code> line shown here:</p>
<div class="code-label " title="bacula-dir.conf — Add Director DirAddress">bacula-dir.conf — Add Director DirAddress</div><pre class="code-pre "><code langs="">Director {                            # define myself
  Name = bacula-dir
  ...
  Messages = Daemon
  <span class="highlight">DirAddress = 127.0.0.1</span>
}
</code></pre>
<p>Now move on to the rest of the file.</p>
...
File = <span class="highlight">/</span>
  ...
  Exclude {
    ...
    File = /.journal
    File = /.fsck
    <span class="highlight">File = /bacula</span>
  }
}
</code></pre>
...
<h3 id="configure-catalog-connection">Configure Catalog Connection</h3>

<p>In the Bacula Director configuration file, the Catalog resource defines where the Database that the Director should use and connect to.</p>

<p>Find the Catalog resource named "MyCatalog" (it's under a comment that says "Generic catalog service"), and update the value of <code>dbpassword</code> so it matches the password you set for the <em>bacula</em> MySQL user:</p>
<div class="code-label " title="bacula-dir.conf — Update Catalog dbpassword">bacula-dir.conf — Update Catalog dbpassword</div><pre class="code-pre "><code langs=""># Generic catalog service
Catalog {
  Name = MyCatalog
# Uncomment the following line if you want the dbi driver
# dbdriver = "dbi:postgresql"; dbaddress = 127.0.0.1; dbport =
  dbname = "bacula"; dbuser = "bacula"; dbpassword = "<span class="highlight">bacula_db_password</span>"
}
</code></pre>
<p>This will allow the Bacula Director to connect to the MySQL database.</p>
...
<h2 id="set-bacula-component-passwords">Set Bacula Component Passwords</h2>

<p>Each Bacula component, such as the Director, SD, and FD, have passwords that are used for inter-component authentication—you probably noticed placeholders while going through the configuration files. It is possible to set these passwords manually but, because you don't actually need to know these passwords, we'll run commands to generate random passwords and insert them into the various Bacula configuration files.</p>

<p>These commands generate and set the Director password. The <code>bconsole</code> connects to the Director, so it needs the password too:</p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">DIR_PASSWORD=`date +%s | sha256sum | base64 | head -c 33`
</li><li class="line" prefix="$">sudo sed -i "s/@@DIR_PASSWORD@@/${DIR_PASSWORD}/" /etc/bacula/bacula-dir.conf
</li><li class="line" prefix="$">sudo sed -i "s/@@DIR_PASSWORD@@/${DIR_PASSWORD}/" /etc/bacula/bconsole.conf
</li></ul></code></pre>
<p>These commands generate and set the Storage Daemon password. The Director connects to the SD, so it needs the password too:</p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">SD_PASSWORD=`date +%s | sha256sum | base64 | head -c 33`
</li><li class="line" prefix="$">sudo sed -i "s/@@SD_PASSWORD@@/${SD_PASSWORD}/" /etc/bacula/bacula-sd.conf
</li><li class="line" prefix="$">sudo sed -i "s/@@SD_PASSWORD@@/${SD_PASSWORD}/" /etc/bacula/bacula-dir.conf
</li></ul></code></pre>
<p>These commands generate and set the local File Daemon (the Bacula client software) password. The Director connects to this FD, so it needs the password too:</p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">FD_PASSWORD=`date +%s | sha256sum | base64 | head -c 33`
</li><li class="line" prefix="$">sudo sed -i "s/@@FD_PASSWORD@@/${FD_PASSWORD}/" /etc/bacula/bacula-dir.conf
</li><li class="line" prefix="$">sudo sed -i "s/@@FD_PASSWORD@@/${FD_PASSWORD}/" /etc/bacula/bacula-fd.conf
</li></ul></code></pre>
<p>Now we're ready to start our Bacula components!</p>

<h2 id="start-bacula-components">Start Bacula Components</h2>

<p>Start the Bacula Director, Storage Daemon, and local File Daemon with these commands:</p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo systemctl start bacula-dir
</li><li class="line" prefix="$">sudo systemctl start bacula-sd
</li><li class="line" prefix="$">sudo systemctl start bacula-fd
</li></ul></code></pre>
<p>If they all started correctly, run these commands so they start automatically on boot:</p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo systemctl enable bacula-dir
</li><li class="line" prefix="$">sudo systemctl enable bacula-sd
</li><li class="line" prefix="$">sudo systemctl enable bacula-fd
</li></ul></code></pre>
<p>Let's test that Bacula works by running a backup job.</p>
...
<p>The next tutorial will show you how to add your other, remote servers as Bacula clients: <a href="https://indiareads/community/tutorials/how-to-back-up-a-centos-7-server-with-bacula">How To Back Up a CentOS 7 Server with Bacula</a>.</p>
