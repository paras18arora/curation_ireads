<link rel='stylesheet' href='digital_ocean.css'><br><img src=https://community-cdn-digitalocean-com.global.ssl.fastly.net/assets/tutorials/images/large/Elasticsearch-twitter.png?1459198120/> <br> 
...
<p>This tutorial will show you how to install and configure a production Elasticsearch cluster on CentOS 7, in a cloud server environment.</p>
...
<p>If you would prefer to use Ubuntu instead, check out this tutorial: <a href="https://indiareads/community/tutorials/how-to-set-up-a-production-elasticsearch-cluster-on-ubuntu-14-04">How To Set Up a Production Elasticsearch Cluster on Ubuntu 14.04</a></p>
...
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">rm ~/jdk-8u73-linux-x64.rpm
</li></ul></code></pre>
...
<p>Be sure to repeat these steps (<a href="https://indiareads/community/tutorials/how-to-set-up-a-production-elasticsearch-cluster-on-centos-7#configure-elasticsearch-cluster">Configure Elasticsearch Cluster</a>) on all of your Elasticsearch servers.</p>
...
<p>Next, open the <code>/etc/sysconfig/elasticsearch</code> file for editing:</p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo vi /etc/sysconfig/elasticsearch
</li></ul></code></pre>
...
<p>The last file to edit is the Elasticsearch systemd unit file. Open it up for editing:</p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo vi /usr/lib/systemd/system/elasticsearch.service
</li></ul></code></pre>
<p>Find and uncomment <code>LimitMEMLOCK=infinity</code>. It should look like this when you're done:</p>
<div class="code-label " title="/usr/lib/systemd/system/elasticsearch.service — LimitMEMLOCK">/usr/lib/systemd/system/elasticsearch.service — LimitMEMLOCK</div><pre class="code-pre "><code langs="">LimitMEMLOCK=infinity
</code></pre>
<p>Save and exit.</p>

<p>Now reload the systemctl daemon and restart Elasticsearch to put the changes into place:</p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo systemctl daemon-reload
</li><li class="line" prefix="$">sudo systemctl restart elasticsearch
</li></ul></code></pre>
<p>Be sure to repeat this step on all of your Elasticsearch servers.</p>
...
<p>Open the <code>/usr/lib/systemd/system/elasticsearch.service</code> file for editing:</p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo vi /usr/lib/systemd/system/elasticsearch.service
</li></ul></code></pre>
<p>Find <code>LimitNOFILE</code> and set it to the limit you desire. For example, if you want a limit of 128k descriptors, change it to <code>131070</code>:</p>
<div class="code-label " title="/usr/lib/systemd/system/elasticsearch.service — LimitNOFILE">/usr/lib/systemd/system/elasticsearch.service — LimitNOFILE</div><pre class="code-pre "><code langs="">LimitNOFILE=<span class="highlight">131070</span>
</code></pre>
<p>Save and exit.</p>
