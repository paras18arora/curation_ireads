<link rel='stylesheet' href='digital_ocean.css'><br><img src=https://community-cdn-digitalocean-com.global.ssl.fastly.net/assets/tutorials/images/large/metrics_topbeat_elk_tw.jpg?1459451530/> <br> 
      <h3 id="introduction">Introduction</h3>

<p>Topbeat, which is one of the several "Beats" data shippers that helps send various types of server data to an Elasticsearch instance, allows you to gather information about the CPU, memory, and process activity on your servers. When used with the ELK stack (Elasticsearch, Logstash, and Kibana), Topbeat can be used as an alternative to other system metrics visualization tools such as <a href="https://indiareads/community/tutorials/how-to-use-prometheus-to-monitor-your-ubuntu-14-04-server">Prometheus</a> or <a href="https://indiareads/community/tutorials/how-to-configure-statsd-to-collect-arbitrary-stats-for-graphite-on-ubuntu-14-04">Statsd</a>.</p>

<p>In this tutorial, we will show you how to use an ELK stack to gather and visualize infrastructure metrics by using <strong>Topbeat</strong> on an Ubuntu 14.04 server.</p>

<h2 id="prerequisites">Prerequisites</h2>

<p>This tutorial assumes that you have the ELK Stack setup described in this tutorial: <a href="https://indiareads/community/tutorials/how-to-install-elasticsearch-logstash-and-kibana-elk-stack-on-ubuntu-14-04">How To Install Elasticsearch, Logstash, and Kibana on Ubuntu 14.04</a>. If you do not already have an ELK server, please complete the linked tutorial before continuing.</p>

<p>We will also assume that, in addition to the ELK server, you have at least one client Ubuntu 14.04 server that you want to gather system metrics from by using Topbeat.</p>
...
<p>Now we can install the Topbeat package.</p>

<h3 id="install-topbeat-package">Install Topbeat Package</h3>

<p>On <strong>Client Server</strong>, ensure that the Beats source list exists. Open <code>/etc/apt/sources.list.d/beats.list</code> for editing:</p>
<pre class="code-pre custom_prefix"><code langs=""><ul class="prefixed"><li class="line" prefix="client$">sudo vi /etc/apt/sources.list.d/beats.list
</li></ul></code></pre>
<p>Ensure that this line exists (paste it in if it isn't already present):</p>
<div class="code-label " title="/etc/apt/sources.list.d/beats.list">/etc/apt/sources.list.d/beats.list</div><pre class="code-pre custom_prefix"><code langs=""><ul class="prefixed"><li class="line" prefix="client$">deb https://packages.elastic.co/beats/apt stable main
</li></ul></code></pre>
<p>Save and exit.</p>

<p>Topbeat uses the same GPG key as Elasticsearch and Filebeat, which can be installed with this command:</p>
...
<p>Then install the Topbeat package:</p>
<pre class="code-pre custom_prefix"><code langs=""><ul class="prefixed"><li class="line" prefix="client$">sudo apt-get update
</li><li class="line" prefix="client$">sudo apt-get install topbeat
</li></ul></code></pre>
<p>Tobeat is now installed but not yet configured.</p>
...
<pre class="code-pre custom_prefix"><code langs=""><ul class="prefixed"><li class="line" prefix="client$">sudo service topbeat restart
</li><li class="line" prefix="client$">sudo update-rc.d topbeat defaults 95 10
</li></ul></code></pre>
