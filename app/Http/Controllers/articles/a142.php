...
<p>In this guide, we will discuss how to install and configure the Bind9 DNS server as a caching or forwarding DNS server on Ubuntu 16.04 machines.  These two configurations both have advantages when serving networks of machines.</p>
...
<p>To follow along, you will need to have access to two computers (at least one of which should be an Ubuntu 16.04 server).  One will function as the client and the other will be configured as the DNS server.  To get the server into a good preliminary state, follow the <a href="https://indiareads/community/tutorials/initial-server-setup-with-ubuntu-16-04">Ubuntu 16.04 initial server setup guide</a>.</p>

<p>The details of our example configuration are:</p>
...
</li><li class="line" prefix="$">sudo apt-get install bind9 bind9utils bind9-doc
</li></ul></code></pre>
...
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">cd /etc/bind
</li></ul></code></pre>
...
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo nano named.conf.options
</li></ul></code></pre>
<p>With the comments stripped out for readability, the file looks like this:</p>
<div class="code-label " title="/etc/bind/named.conf.options">/etc/bind/named.conf.options</div><pre class="code-pre "><code langs="">options {
...
<p>Above the <code>options</code> block, we will create a new block called <code>acl</code>.  Create a label for the ACL group that you are configuring.  In this guide, we will call the group <strong>goodclients</strong>.</p>
<div class="code-label " title="/etc/bind/named.conf.options">/etc/bind/named.conf.options</div><pre class="code-pre "><code langs=""><span class="highlight">acl goodclients {</span>
<span class="highlight">};</span>

options {
    . . .
</code></pre>
<p>Within this block, list the IP addresses or networks that should be allowed to use this DNS server.  Since both our server and client are operating within the same /24 subnet in our example, we will restrict the example to this network.  You should adjust this to include your own clients, and no outside parties.  We will also add <code>localhost</code> and <code>localnets</code> which will attempt to do this automatically:</p>
<div class="code-label " title="/etc/bind/named.conf.options">/etc/bind/named.conf.options</div><pre class="code-pre "><code langs="">acl goodclients {
    <span class="highlight">192.0.2.0/24;</span>
    <span class="highlight">localhost;</span>
    <span class="highlight">localnets;</span>
};
...
<div class="code-label " title="/etc/bind/named.conf.options">/etc/bind/named.conf.options</div><pre class="code-pre "><code langs="">. . .
...
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo systemctl restart bind9
</li></ul></code></pre>
<p>If you followed the initial server setup guide, the UFW firewall is enabled on your server.  We need to allow DNS traffic to our server in order to respond to client requests.</p>

<p>Enable an exception to the firewall policy for Bind by typing:</p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo ufw allow Bind9
</li></ul></code></pre>
...
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo journalctl -u bind9 -f
</li></ul></code></pre>
...
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo nano /etc/resolv.conf
</li></ul></code></pre>
...
<div class="code-label " title="/etc/resolv.conf">/etc/resolv.conf</div><pre class="code-pre "><code langs="">nameserver <span class="highlight">192.0.2.2</span>
# nameserver 8.8.4.4
# nameserver 8.8.8.8
...
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">ping -c 1 google.com
</li></ul></code></pre><pre class="code-pre "><code langs=""><div class="secondary-code-label " title="Output">Output</div>PING google.com (173.194.33.1) 56(84) bytes of data.
...
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">dig linuxfoundation.org
</li></ul></code></pre><pre class="code-pre "><code langs=""><div class="secondary-code-label " title="Output">Output</div>; <<>> DiG 9.9.5-3-Ubuntu <<>> linuxfoundation.org
;; global options: +cmd
;; Got answer:
...
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">dig -x 140.211.169.4
</li></ul></code></pre><pre class="code-pre "><code langs=""><div class="secondary-code-label " title="Output">Output</div>; <<>> DiG 9.9.5-3-Ubuntu <<>> -x 140.211.169.4
;; global options: +cmd
;; Got answer:
...
<pre class="code-pre "><code langs=""><div class="secondary-code-label " title="Output from sudo journalctl -u bind9 -f">Output from sudo journalctl -u bind9 -f</div>. . .
...
<p>To do this, we can modify the systemd unit file that starts Bind9:</p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo systemctl edit --full bind9
</li></ul></code></pre>
<p>Inside the file that appears, add <code>-4</code> to the end of the <code>ExecStart</code> line to restrict the server to IPv4 requests:</p>
<div class="code-label " title="Editing bind9 systemd unit file">Editing bind9 systemd unit file</div><pre class="code-pre "><code langs="">[Unit]
...
<p>Reload the systemd daemon to read the changed unit file into the init system:</p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo systemctl daemon-reload
</li></ul></code></pre>
<p>Restart the Bind9 service to implement the changes:</p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo systemctl restart bind9
</li></ul></code></pre>
...
iface eth0 inet static
        ...
        gateway 192.168.2.1
        dns-nameservers <span class="highlight">192.0.2.2</span>
...
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo nano /etc/sysconfig/network-scripts/ifcfg-eth0
</li></ul></code></pre>
...
DNS1=<span class="highlight">192.0.2.2</span>
. . .
</code></pre>
