<link rel='stylesheet' href='digital_ocean.css'><br><img src=https://community-cdn-digitalocean-com.global.ssl.fastly.net/assets/tutorials/images/large/apache_caching_tw.jpg?1429024926/> <br> 
...
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo nano /etc/httpd/conf.modules.d/00-cache.conf
</li></ul></code></pre>
...
<div class="code-label " title="/etc/httpd/conf.modules.d/00-cache.conf">/etc/httpd/conf.modules.d/00-cache.conf</div><pre class="code-pre "><code class="code-highlight language-apache">LoadModule file_cache_module modules/mod_file_cache.so
</code></pre>
...
<div class="code-label " title="/etc/httpd/conf/httpd.conf">/etc/httpd/conf/httpd.conf</div><pre class="code-pre "><code class="code-highlight language-apache">CacheFile /var/www/html/index.html /var/www/html/somefile.index
</code></pre>
...
<div class="code-label " title="/etc/httpd/conf/httpd.conf">/etc/httpd/conf/httpd.conf</div><pre class="code-pre "><code class="code-highlight language-apache">MMapFile /var/www/html/index.html /var/www/html/somefile.index
</code></pre>
...
<div class="code-label " title="/etc/httpd/conf/httpd.conf">/etc/httpd/conf/httpd.conf</div><pre class="code-pre "><code class="code-highlight language-apache">AuthnCacheSOCache shmcb
...
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo nano /etc/httpd/conf.d/site.conf
</li></ul></code></pre>
<p>A basic virtual host set up with authentication may look something like this:</p>
<div class="code-label " title="/etc/httpd/conf.d/site.conf">/etc/httpd/conf.d/site.conf</div><pre class="code-pre "><code class="code-highlight language-apache"><VirtualHost *:80>
...
<div class="code-label " title="/etc/httpd/conf.d/ssl.conf">/etc/httpd/conf.d/ssl.conf</div><pre class="code-pre "><code class="code-highlight language-apache">. . .
...
<h4 id="setting-up-htcacheclean-to-automatically-manage-the-cache">Setting Up htcacheclean to Automatically Manage the Cache</h4>

<p>On a CentOS 7 system, the <code>htcacheclean</code> utility, which is used to pare down the cache as it grows, is installed during the <code>httpd</code> installation.  A <code>systemd</code> unit file called <code>htcacheclean.service</code> is included by default.</p>

<p>If you plan on setting up caching, it is a good idea to configure this service to run automatically.  The service file actually daemonizes the service, running the cleaning operation at a configurable interval.  However, by default, there is not a mechanism for starting it when Apache starts.</p>

<p>To set this up, we'll make a directory called <code>httpd.service.requires</code> within the <code>/etc/systemd/system</code> directory.  This can be used to specify dependencies of the <code>httpd.service</code> unit file that starts Apache:</p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo mkdir -p /etc/systemd/system/httpd.service.requires
</li></ul></code></pre>
<p>Afterwards, we can link the <code>htcacheclean.service</code> unit file into this directory.  This will cause the <code>htcacheclean</code> service to start and clean the cache at regular intervals when Apache is started:</p>
<pre class="code-pre "><code langs="">sudo ln -s /usr/lib/systemd/system/htcacheclean.service /etc/systemd/system/httpd.service.requires
</code></pre>
<p>You can configure the <code>htcacheclean</code> options, including the cleaning interval, by editing the <code>htcacheclean</code> file in the <code>/etc/sysconfig</code> directory:</p>
<pre class="code-pre "><code langs="">sudo nano /etc/sysconfig/htcacheclean
</code></pre>
<p>Here, you can modify the cleaning interval, the cache root, the maximum cache size, and any other options for the utility:</p>
<div class="code-label " title="/etc/sysconfig/htcacheclean">/etc/sysconfig/htcacheclean</div><pre class="code-pre "><code langs="">INTERVAL=15
CACHE_ROOT=/var/cache/httpd/proxy
LIMIT=100M
OPTIONS=
</code></pre>
<p></p><div class="code-label notes-and-warnings note" title="Note">Note</div><span class="note">
Keep in mind that if you change the value of <code>CACHE_ROOT</code>, you will need to adjust the value the <code>CacheRoot</code> directive that we will be setting in our Apache configuration.<br /></span>

<p>When you are finished, save and close the file.</p>

<p>Restart Apache to start the <code>htcacheclean</code> to automatically clean the cache:</p>
<pre class="code-pre "><code langs="">sudo systemctl restart httpd
</code></pre>
<h4 id="modifying-the-global-configuration">Modifying the Global Configuration</h4>

<p>Most of the configuration for caching will take place within individual virtual host definitions or location blocks.  However, there are also some global configuration items that should be used to set up some general attributes.  Open your main Apache config file to configure these items:</p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo nano /etc/httpd/conf/httpd.conf
</li></ul></code></pre>
<p>We need to add the <code>CacheRoot</code> directory to point to the path that should be used to store our cached items.  This should always match the <code>CACHE_ROOT</code> value found in your <code>/etc/sysconfig/htcacheclean</code> file so that the cache can be managed correctly.  We will also set the <code>CacheDirLevels</code> and <code>CacheDirLength</code> directives, which both contribute towards defining how the cache directory structure will be built:</p>
<div class="code-label " title="/etc/httpd/conf/httpd.conf">/etc/httpd/conf/httpd.conf</div><pre class="code-pre "><code class="code-highlight language-apache">CacheRoot /var/cache/httpd/proxy
CacheDirLevels 2
CacheDirLength 1
</code></pre>
<p>The <code>CacheDirLevels</code> and <code>CacheDirLength</code> both contribute towards defining how the cache directory structure will be built.  An <code>md5</code> hash of the URL being served will be created as the key used to store the data.  The data will be organized into directories derived from the beginning characters of each hash.  <code>CacheDirLevels</code> specifies the number of subdirectories to create and <code>CacheDirLength</code> specifies how many characters to use as the name of each directory.  So a hash of <code>b1946ac92492d2347c6235b4d2611184</code> with the default values shown above would be filed in a directory structure of <code>b/1/946ac92492d2347c6235b4d2611184</code>.  Usually, you won't need to modify these values, but it's good to know what they're used for.</p>
...
<p>Open one of your virtual host files to follow along.  We'll assume you're using a file called <code>site.conf</code> within your <code>/etc/httpd/conf.d</code> directory in this guide:</p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo nano /etc/httpd/conf.d/site.conf
</li></ul></code></pre>
