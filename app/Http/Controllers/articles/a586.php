...
<p>In this guide, we'll demonstrate how to install and configure MySQL or MariaDB to use with your Django applications.  We will install the necessary software, create database credentials for our application, and then start and configure a new Django project to use this backend.</p>

<h2 id="prerequisites">Prerequisites</h2>

<p>To get started, you will need a clean Ubuntu 14.04 server instance with a non-root user set up.  The non-root user must be configured with <code>sudo</code> privileges.  Learn how to set this up by following our <a href="https://indiareads/community/tutorials/initial-server-setup-with-ubuntu-14-04">initial server setup guide</a>.</p>
...
<p>We will cover both MySQL and MariaDB below, so choose the section associated with the DBMS you'd like to use.</p>
...
<p>If you prefer MariaDB, you can follow the instructions below to install it and perform the necessary initial configuration.  Install the packages from the repositories by typing:</p>
<pre class="code-pre "><code langs="">sudo apt-get update
sudo apt-get install python-pip python-dev mariadb-server libmariadbclient-dev libssl-dev
</code></pre>
<p>You will be asked to select and confirm a password for the administrative MariaDB account.</p>

<p>You can then run through a simple security script by running:</p>
<pre class="code-pre "><code langs="">sudo mysql_secure_installation
</code></pre>
<p>You'll be asked for the administrative password you set for MariaDB during installation.  Afterwards, you'll be asked a series of questions.  Besides the first question, asking you to choose another administrative password, select yes for each question.</p>
...
<p>This is currently configured to use SQLite as a database.  We need to change this so that our MySQL/MariaDB database is used instead.</p>
...
<p>In this guide, we've demonstrated how to install and configure MySQL or MariaDB as the backend database for a Django project.  While SQLite can easily handle the load during development and light production use, most projects benefit from implementing a more full-featured DBMS.</p>
