...
<p>This tutorial will show you how to set up Bacula to create backups of a remote CentOS 7 host, over a network connection. This involves installing and configuring the Bacula Client software on a remote host, and making some additions to the configuration of an existing Bacula Server (covered in the prerequisites).</p>

<p>If you are trying to create backups of Ubuntu 14.04 hosts, follow this link instead: <a href="https://indiareads/community/tutorials/how-to-back-up-an-ubuntu-14-04-server-with-bacula">How To Back Up an Ubuntu 14.04 Server with Bacula</a>.</p>

<h2 id="prerequisites">Prerequisites</h2>

<p>This tutorial assumes that you have a server running the Bacula Server components, as described in this link: <a href="https://indiareads/community/tutorials/how-to-install-bacula-server-on-centos-7">How To Install Bacula Server on CentOS 7</a>.</p>
...
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo yum install bacula-client
</li></ul></code></pre>
...
<p>We will need to set a password, that will allow the Bacula Director to connect, in the File Daemon configuration. Let's generate a random password now (or you can skip this step and make up your own password):</p>
<pre class="code-pre "><code langs="">date +%s | sha256sum | base64 | head -c 33 ; echo
</code></pre>
<p>You will want to copy the output, as you will use it in the next step.</p>
...
<p>Begin by finding the Director resource that is named "ClientHost-dir". As the Bacula Director that we want to control this Client is located on the Bacula Server, change the "Name" parameter to the hostname of your backup server followed by "-dir". Also, replace the existing password with the one you generated in the previous step. It should look something like this after being updated:</p>
<div class="code-label " title="bacula-fd.conf — Update Director Name and Password">bacula-fd.conf — Update Director Name and Password</div><pre class="code-pre "><code langs="">Director {
  Name = <span class="highlight">BackupServer</span>-dir
  Password = "<span class="highlight">Y2Q5ODUyMWM0YTFhYjA3NTcwYmU5OTA4Y</span>"
}
</code></pre>
<p>Be sure to keep the password handy. It will be used in the Backup Server's Director configuration, which we will set in an upcoming step, to connect to your Client's File Daemon.</p>
...
<div class="code-label " title="bacula-fd.conf — Update Name and Add FDAddress">bacula-fd.conf — Update Name and Add FDAddress</div><pre class="code-pre "><code langs="">FileDaemon {                          # this is me
  Name = <span class="highlight">ClientHost</span>-fd
  FDAddress = <span class="highlight">client_private_ip</span>
...
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo systemctl restart bacula-fd
</li></ul></code></pre>
<p>Then run the following command to start the Bacula File Daemon automatically on boot:</p>
<pre class="code-pre "><code langs="">sudo systemctl enable bacula-fd
</code></pre>
...
<div class="code-label " title="conf.d/clients.conf — Add Client resource">conf.d/clients.conf — Add Client resource</div><pre class="code-pre "><code langs="">Client {
  ...
  FDPort = 9102 
  Catalog = MyCatalog
  Password = "<span class="highlight">Y2Q5ODUyMWM0YTFhYjA3NTcwYmU5OTA4Y</span>"          # password for Remote FileDaemon
...
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo bacula-dir /etc/bacula/bacula-dir.conf
</li></ul></code></pre>
...
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo systemctl restart bacula-dir
</li></ul></code></pre>
...
<p>You now have a Bacula Server that is backing up files from a remote Bacula Client. Be sure to review and revise your configuration until you are certain that you are backing up the correct FileSets, on a schedule that meets your needs. If you are trying to create backups of Ubuntu 14.04 hosts, follow this link: <a href="https://indiareads/community/tutorials/how-to-back-up-an-ubuntu-14-04-server-with-bacula">How To Back Up an Ubuntu 14.04 Server with Bacula</a>.</p>

<p>The next thing you should do is repeat the relevant sections of this tutorial for any additional CentOS 7 servers that you want to back up.</p>
