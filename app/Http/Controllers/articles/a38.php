...
<ul>
<li><p>One CentOS 7 Droplet with a sudo non-root user, which you can set up by following <a href="https://indiareads/community/tutorials/initial-server-setup-with-centos-7">this initial server setup tutorial</a>.</p></li>
<li><p>Nginx installed and configured on your server, which you can do by following <a href="https://indiareads/community/tutorials/how-to-install-nginx-on-centos-7">this Nginx installation tutorial</a>.</p></li>
</ul>

<h2 id="step-1-—-installing-httpd-tools">Step 1 — Installing HTTPD Tools</h2>

<p>You'll need the <code>htpassword</code> command to configure the password that will restrict access to the target website. This command is part of the <code>httpd-tools</code> package, so the first step is to install that package.</p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo yum install -y httpd-tools
</li></ul></code></pre>
...
<p>To create the password, run the following command.</p>
...
<p>Both directives should be in the configuration file of the target website, which is normally located in the <code>/etc/nginx/</code> directory. Open that file using <code>nano</code> or your favorite text editor.</p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo nano /etc/nginx/nginx.conf
</li></ul></code></pre>
<p>Under the <strong>server</strong> section, add both directives:</p>
