<link rel='stylesheet' href='digital_ocean.css'><br><img src=https://community-cdn-digitalocean-com.global.ssl.fastly.net/assets/tutorials/images/large/wordpress_tw.png?1461609276/> <br> 
...
<p>In this guide, we'll focus on getting a WordPress instance set up on a LAMP stack (Linux, Apache, MySQL, and PHP) on an Ubuntu 16.04 server.</p>
...
<li><strong>Install a LAMP stack</strong>: WordPress will need a web server, a database, and PHP in order to correctly function.  Setting up a LAMP stack (Linux, Apache, MySQL, and PHP) fulfills all of these requirements.  Follow <a href="https://indiareads/community/tutorials/how-to-install-linux-apache-mysql-php-lamp-stack-on-ubuntu-16-04">this guide</a> to install and configure this software.</li>
...
<li><strong>If you do not have a domain...</strong> and you are just using this configuration for testing or personal use, you can use a self-signed certificate instead.  This provides the same type of encryption, but without the domain validation.  Follow our <a href="https://indiareads/community/tutorials/how-to-create-a-self-signed-ssl-certificate-for-apache-in-ubuntu-16-04">self-signed SSL guide for Apache</a> to get set up.</li>
</ul></li>
</ul>
...
<h2 id="step-2-install-additional-php-extensions">Step 2:  Install Additional PHP Extensions</h2>
...
<p>We will restart Apache to leverage these new extensions in the next section.  If you are returning here to install additional plugins, you can restart Apache now by typing:</p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo systemctl restart apache2
</li></ul></code></pre>
<h2 id="step-3-adjust-apache-39-s-configuration-to-allow-for-htaccess-overrides-and-rewrites">Step 3: Adjust Apache's Configuration to Allow for .htaccess Overrides and Rewrites</h2>

<p>Next, we will be making a few minor adjustments to our Apache configuration.  Currently, the use of <code>.htaccess</code> files is disabled.  WordPress and many WordPress plugins use these files extensively for in-directory tweaks to the web server's behavior.</p>

<p>Additionally, we will enable <code>mod_rewrite</code>, which will be needed in order to get WordPress permalinks to function correctly.</p>

<h3 id="enable-htaccess-overrides">Enable .htaccess Overrides</h3>

<p>Open the primary Apache configuration file to make our first change:</p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo nano /etc/apache2/apache2.conf
</li></ul></code></pre>
<p>To allow <code>.htaccess</code> files, we need to set the <code>AllowOverride</code> directive within a <code>Directory</code> block pointing to our document root.  Towards the bottom of the file, add the following block:</p>
...
<h3 id="enable-the-rewrite-module">Enable the Rewrite Module</h3>

<p>Next, we can enable <code>mod_rewrite</code> so that we can utilize the WordPress permalink feature:</p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo a2enmod rewrite
</li></ul></code></pre>
<h3 id="enable-the-changes">Enable the Changes</h3>

<p>Before we implement the changes we've made, check to make sure we haven't made any syntax errors:</p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo apache2ctl configtest
</li></ul></code></pre>
<p>The output might have a message that looks like this:</p>
...
<p>If you wish to suppress the top line, just add a <code>ServerName</code> directive to the <code>/etc/apache2/apache2.conf</code> file pointing to your server's domain or IP address.  This is just a message however and doesn't affect the functionality of our site.  As long as the output contains <code>Syntax OK</code>, you are ready to continue.</p>

<p>Restart Apache to implement the changes:</p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo systemctl restart apache2
</li></ul></code></pre>
...
<p>We will be moving these files into our document root momentarily.  Before we do, we can add a dummy <code>.htaccess</code> file and set its permissions so that this will be available for WordPress to use later.</p>

<p>Create the file and set the permissions by typing:</p>
...
<p>We'll also copy over the sample configuration file to the filename that WordPress actually reads:</p>
...
<p>Now, we can copy the entire contents of the directory into our document root.  We are using the <code>-a</code> flag to make sure our permissions are maintained.  We are using a dot at the end of our source directory to indicate that everything within the directory should be copied, including hidden files (like the <code>.htaccess</code> file we created):</p>
