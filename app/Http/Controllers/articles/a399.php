...
<p>The following <code>apt</code> commands will get you the packages you need:</p>
...
<p>First, change the engine so that it uses the <code>postgresql_psycopg2</code> backend instead of the <code>sqlite3</code> backend.  For the <code>NAME</code>, use the name of your database (<code><span class="highlight">myproject</span></code> in our example).  We also need to add login credentials.  We need the username, password, and host to connect to.  We'll add and leave blank the port option so that the default is selected:</p>
<pre class="code-pre "><code langs="">. . .
