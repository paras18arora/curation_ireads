<link rel='stylesheet' href='digital_ocean.css'><br><img src=https://community-cdn-digitalocean-com.global.ssl.fastly.net/assets/tutorials/images/large/rediscluster.png?1440439087/> <br> 
...
<p>While this may work on earlier releases and other Linux distributions, we recommend using CentOS 7. </p>
...
<li>Access to your machines via SSH with a sudo non-root user as explained in <a href="https://indiareads/community/tutorials/initial-server-setup-with-centos-7">Initial Server Setup with CentOS 7</a></li>
</ul>

<h2 id="step-1-—-install-redis">Step 1 — Install Redis</h2>

<p>Starting with the Droplet that will host our <strong>master server</strong>, our first step is to install Redis. First we need to enable the EPEL repository on our machine. If you are unfamiliar with it, EPEL is the Extra Packages for Enterprise Linux repo, developed by the Fedora project with the intention of providing quality third-party packages for enterprise users of RHEL-based distros.</p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">wget http://dl.fedoraproject.org/pub/epel/7/x86_64/e/epel-release-7-5.noarch.rpm
</li></ul></code></pre>
<p>If <code>wget</code> is not recognized, try running <code>yum install wget</code> before the above command.</p>

<p>Now run:</p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo rpm -ivh epel-release-7-5.noarch.rpm
</li></ul></code></pre>
<p>And now type in:</p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo yum -y update
</li></ul></code></pre>
<p>Note that this may take a while to complete. Now you may install Redis on your machine, by running: </p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo yum install redis -y
</li></ul></code></pre>
<p>Once the installation process has finished, starting the Redis service is done by entering the following command:</p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo systemctl start redis.service
</li></ul></code></pre>
<p>And checking its status can be done with the following command:</p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo systemctl status redis.service
</li></ul></code></pre>
<p>Which outputs something similar to:</p>
<pre class="code-pre "><code langs=""><div class="secondary-code-label " title="Output">Output</div><span class="highlight">redis.service - Redis persistent key-value database</span>
<span class="highlight">   Loaded: loaded (/usr/lib/systemd/system/redis.service; disabled)</span>
<span class="highlight">  Drop-In: /etc/systemd/system/redis.service.d</span>
<span class="highlight">           └─limit.conf</span>
<span class="highlight">   Active: active (running) since Wed 2015-07-22 02:26:31 EDT; 13s ago</span>
<span class="highlight"> Main PID: 18995 (redis-server)</span>
<span class="highlight">   CGroup: /system.slice/redis.service</span>
 <span class="highlight">          └─18995 /usr/bin/redis-server 127.0.0.1:6379</span>
</code></pre>
<p>Finally, let's test our Redis setup by running:</p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">redis-cli ping
</li></ul></code></pre>
<p>This should print a <code>PONG</code> as the response. If this is the case, you now have Redis running on your server, and we can start configuring it. An additional test for our setup can be done by running:</p>
...
<span class="highlight">SET: 200000.00 requests per second</span>
<span class="highlight">GET: 200000.00 requests per second</span>
<span class="highlight">INCR: 200000.00 requests per second</span>
<span class="highlight">LPUSH: 200000.00 requests per second</span>
<span class="highlight">LPOP: 200000.00 requests per second</span>
<span class="highlight">SADD: 200000.00 requests per second</span>
<span class="highlight">SPOP: 249999.98 requests per second</span>
<span class="highlight">LPUSH (needed to benchmark LRANGE): 200000.00 requests per second</span>
<span class="highlight">LRANGE_100 (first 100 elements): 35714.29 requests per second</span>
<span class="highlight">LRANGE_300 (first 300 elements): 11111.11 requests per second</span>
<span class="highlight">LRANGE_500 (first 450 elements): 7194.24 requests per second</span>
<span class="highlight">LRANGE_600 (first 600 elements): 5050.50 requests per second</span>
<span class="highlight">MSET (10 keys): 100000.00 requests per second</span>
</code></pre>
...
<p>Open <code>/etc/redis.conf</code> with your favorite text editor:</p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo vi /etc/redis.conf
</li></ul></code></pre>
<p>Edit the following lines.</p>

<p>Set a sensible value to the keepalive timer for TCP:</p>
<div class="code-label " title="/etc/redis.conf">/etc/redis.conf</div><pre class="code-pre "><code langs="">tcp-keepalive <span class="highlight">60</span>
</code></pre>
<p>Make the server accessible to anyone on the web by commenting out this line:</p>
<div class="code-label " title="/etc/redis.conf">/etc/redis.conf</div><pre class="code-pre "><code langs=""><span class="highlight">#</span>bind 127.0.0.1
</code></pre>
...
<div class="code-label " title="/etc/redis.conf">/etc/redis.conf</div><pre class="code-pre "><code langs="">requirepass <span class="highlight">your_redis_master_password</span>
</code></pre>
...
<div class="code-label " title="/etc/redis.conf">/etc/redis.conf</div><pre class="code-pre "><code langs="">maxmemory-policy <span class="highlight">noeviction</span>
</code></pre>
<p>Finally, we want to make the following changes, required for backing up data. Uncomment and/or set these lines as shown:</p>
<div class="code-label " title="/etc/redis.conf">/etc/redis.conf</div><pre class="code-pre "><code langs="">appendonly <span class="highlight">yes</span>
appendfilename "<span class="highlight">appendonly.aof</span>"
</code></pre>
...
<div class="code-label " title="/etc/redis.conf">/etc/redis.conf</div><pre class="code-pre "><code langs="">requirepass <span class="highlight">your_redis_slave_password</span>
</code></pre>
...
<div class="code-label " title="/etc/redis.conf">/etc/redis.conf</div><pre class="code-pre "><code langs="">slaveof <span class="highlight">your_redis_master_ip 6379</span>
</code></pre>
...
<div class="code-label " title="/etc/redis.conf">/etc/redis.conf</div><pre class="code-pre "><code langs="">masterauth <span class="highlight">your_redis_master_password</span>
</code></pre>
...
<p>Next steps might include scripting the automated failover procedure, or ensuring secure communications between all your Droplets by the use of VPN solutions such as <a href="https://indiareads/community/tutorials/how-to-setup-and-configure-an-openvpn-server-on-centos-7">OpenVPN</a>. Also, testing procedures and scripts are vital for validating your configurations.</p>
