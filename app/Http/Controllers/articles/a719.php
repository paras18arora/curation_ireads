...
<p>This tutorial will explain how to install and get started with Composer on an Ubuntu 16.04 system.</p>
...
<ul>
<li>A system running Ubuntu 14.04</li>
...
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo apt-get install curl php-cli php-mbstring git unzip
</li></ul></code></pre>
...
<p>Composer provides an installer, written in PHP.  Make sure you're in your home directory, and retrieve the installer using <code>curl</code>:</p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">cd ~
</li><li class="line" prefix="$">curl -sS https://getcomposer.org/installer -o composer-setup.php
</li></ul></code></pre>
<p>Next, run a short PHP script to verify that the installer matches the SHA-384 hash for the latest installer found on the <a href="https://composer.github.io/pubkeys.html">Composer Public Keys / Signatures</a> page.  You will need to make sure that you substitute the latest hash for the highlighted value below:</p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">php -r "if (hash_file('SHA384', 'composer-setup.php') === '<span class="highlight">92102166af5abdb03f49ce52a40591073a7b859a86e8ff13338cf7db58a19f7844fbc0bb79b2773bf30791e935dbd938</span>') { echo 'Installer verified'; } else { echo 'Installer corrupt'; unlink('composer-setup.php'); } echo PHP_EOL;"
</li></ul></code></pre><div class="code-label " title="Output">Output</div><pre class="code-pre "><code langs="">Installer verified
</code></pre>
<p>To install <code>composer</code> globally, use the following:</p>
<pre class="code-pre "><code langs="">sudo php composer-setup.php --install-dir=/usr/local/bin --filename=composer
</code></pre>
...
<p><span class="note">If you prefer to have separate Composer executables for each project you host on this server, you can simply install it locally, on a per-project basis.  Users of NPM will be familiar with this approach.  This method is also useful when your system user doesn't have permission to install software system-wide.<br /></span></p>

<p>In this case, installation can be done, after downloading and verifying the installation script as above, like so:</p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">php composer-setup.php
</li></ul></code></pre>
<p>This will generate a <code>composer.phar</code> file in your current directory, which can be executed with <code>./composer.phar <span class="highlight">command</span></code>.</p>
...
</li></ul></code></pre><pre class="code-pre "><code langs=""><div class="secondary-code-label " title="Output">Output</div>Using version ^2.1 for cocur/slugify
...
<p>For a more in-depth view of Composer version constraints, read <a href="https://getcomposer.org/doc/articles/versions.md">the official documentation</a>.</p>

<h2 id="step-4-—-including-the-autoload-script">Step 4 — Including the Autoload Script</h2>

<p>Since PHP itself doesn't automatically handle the loading of classes, Composer also provides an autoload script that you can include in your project to get autoloading for free. This makes it much easier to work with your dependencies and define your own namespaces.</p>
...
<p>Let's come back to the <em>slugify</em> example application. We'll use <code>nano</code> to create a <code>test.php</code> script where we'll use the <em>cocur/slugify</em> library:</p>
...
</li><li class="line" prefix="2">require __DIR__ . '/vendor/autoload.php';
...
<p>This tutorial covered the essentials for getting started with Composer on Ubuntu 16.04.  Composer is a powerful tool every PHP developer should have in their utility belt.</p>
