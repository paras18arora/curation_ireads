<link rel='stylesheet' href='digital_ocean.css'><br><img src=https://community-cdn-digitalocean-com.global.ssl.fastly.net/assets/tutorials/images/large/mysql_databases_tw__2_.jpg?1430403792/> <br> 
...
<p>As opposed to the <em>logical backups</em> that utilities like mysqldump produce, XtraBackup creates <em>physical backups</em> of the database files—it makes a copy of the data files. Then it applies the transaction log (a.k.a. redo log) to the physical backups, to backfill any active transactions that did not finish during the creation of the backups, resulting in consistent backups of a running database. The resulting database backup can then be backed up to a remote location using <a href="https://indiareads/community/tutorials/how-to-use-rsync-to-sync-local-and-remote-directories-on-a-vps">rsync</a>,  a backup system like <a href="https://indiareads/community/tutorials/how-to-install-bacula-server-on-centos-7">Bacula</a>, or <a href="https://indiareads/community/tutorials/understanding-digitalocean-droplet-backups">IndiaReads backups</a>.</p>

<p>This tutorial will show you how to perform a full hot backup of your MySQL or MariaDB databases using Percona XtraBackup on CentOS 7. The process of restoring the database from a backup is also covered. The Ubuntu version of this guide can be found <a href="https://indiareads/community/tutorials/how-to-create-hot-backups-of-mysql-databases-with-percona-xtrabackup-on-ubuntu-14-04">here</a>.</p>
...
<ul>
<li>Superuser privileges on an CentOS 7 system</li>
...
<p>The easiest way to install Percona XtraBackup is to use yum, as Percona's repository provides an RPM.</p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo yum install http://www.percona.com/downloads/percona-release/redhat/0.1-3/percona-release-0.1-3.noarch.rpm
</li></ul></code></pre>
<p>Then, you can run this command to install XtraBackup:</p>
<pre class="code-pre command"><code langs=""><ul class="prefixed"><li class="line" prefix="$">sudo yum install percona-xtrabackup
</li></ul></code></pre>
<p>Accept any confirmation prompts to complete the installation.</p>
...
<p>First of all, it is advisable to automate the process so you will have backups created according to a schedule. Second, you should make remote copies of the backups, in case your database server has problems, by using something like <a href="https://indiareads/community/tutorials/how-to-use-rsync-to-sync-local-and-remote-directories-on-a-vps">rsync</a>, a network file backup system like <a href="https://indiareads/community/tutorials/how-to-install-bacula-server-on-centos-7">Bacula</a>, or <a href="https://indiareads/community/tutorials/understanding-digitalocean-droplet-backups">IndiaReads backups</a>. After that, you will want to look into <strong>rotating</strong> your backups (deleting old backups on a schedule) and creating incremental backups (with XtraBackup) to save disk space.</p>
