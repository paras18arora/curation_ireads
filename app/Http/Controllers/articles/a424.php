...
<p>When you first log into a fresh Fedora 22, it's not ready for use as a production system. There are a number of recommended steps to take in order to customize and secure it, such as enabling a firewall.</p>

<p>This tutorial will show you how to give a fresh installation of a Fedora 22 server a better security profile and be ready for use.</p>
...
<ul>
<li>A Fedora 22 Droplet with root SSH keys.</li>
</ul>
...
<pre class="code-pre "><code langs="">sudo dnf install -y nano
</code></pre>
...
<p>A new Fedora 22 server has no active firewall application. In this step, we'll learn how to enable the IPTables firewall application and make sure that runtime rules persist after a reboot.</p>
...
<pre class="code-pre "><code langs="">sudo dnf install -y iptables-services
</code></pre>
...
<p>IPTables on Fedora 22 ships with a default set of rules. One of those rules permits SSH traffic. To view the default rules, type:</p>
...
<pre class="code-pre "><code langs="">sudo dnf install -y mlocate
</code></pre>
...
<p>After completing the last step, your Fedora 22 server should be configured, reasonably secure, and ready for use!</p>
